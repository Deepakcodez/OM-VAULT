...
const SmallStatCards: React.FC<SmallStatCardsProps> = ({ title, value, stat, percentage }) => {
  return (
    <div className='flex gap-12  border border-t-neutral-500 border-neutral-700 rounded-2xl p-4 w-fit bg-gradient-to-b from-neutral-700/50 select-none '>
      <div>
      ...
      <div className={`relative flex justify-center items-center w-24 rounded-2xl bg-gradient-to-t ${stat == 'up' ? 'from-green-300/10' : stat=='down'? 'from-red-300/10':'from-blue-300/10'}`}>
        {
          stat == 'up' ?
            <>
